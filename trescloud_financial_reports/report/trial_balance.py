...
class TrialBalanceReport(models.AbstractModel):
    _name = "report.trescloud_financial_reports.trial_balance"
    ...
    def _get_html(self):
        ...
        result["html"] = self.env.ref(
            "trescloud_financial_reports.report_trial_balance"
        ).render(rcontext)
        return result
    ...
    def _get_data(
        ...
        if unaffected_id and unaffected_id not in accounts_ids:
    ...
    def _get_hierarchy_groups(
        self, group_ids, groups_data, old_groups_ids, foreign_currency, pendent_groups, level
    ):
        new_parents = False
        new_pendents = []
        for group_id in group_ids + pendent_groups:
            if groups_data[group_id]["parent_id"]:
                nw_id_obj = self.env["account.group"].browse(groups_data[group_id]["parent_id"])
                if len(nw_id_obj.code_prefix_start) == level:
                    new_parents = True
                    ...
                    new_pendents.append(group_id)
        if new_parents or new_pendents:
            nw_groups_ids = []
            ...
            groups = self.env["account.group"].browse(nw_groups_ids + new_pendents)
            for group in groups:
            ...
            groups_data = self._get_hierarchy_groups(
                nw_groups_ids, groups_data, old_groups_ids, foreign_currency, new_pendents, level - 1
            )
        return groups_data
    ...
    def _get_groups_data(self, accounts_data, total_amount, foreign_currency):
        ...
        max_group_level = max(
            self.env["account.group"].browse(group_ids).mapped("code_prefix_start"), key=lambda rec: len(rec)
        )
        groups_data = self._get_hierarchy_groups(
            group_ids, groups_data, old_group_ids, foreign_currency, [], len(max_group_level) - 1
        )
        return groups_data
    ...
    def _get_summarized_report_values(self, docids, data):
        show_partner_details = data["show_partner_details"]
        company_id = data["company_id"]
        partner_ids = data["partner_ids"]
        journal_ids = data["journal_ids"]
        ...
        for account_id in accounts_data.keys():
            group_data["initial_balance"] += total_amount[account_id][
                "initial_balance"
            ]
            group_data["debit"] += total_amount[account_id]["debit"]
            group_data["credit"] += total_amount[account_id]["credit"]
            group_data["balance"] += total_amount[account_id]["balance"]
            group_data["ending_balance"] += total_amount[account_id][
                "ending_balance"
            ]
        return group_data
