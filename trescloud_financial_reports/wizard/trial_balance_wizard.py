...
class TrialBalanceReportWizard(models.TransientModel):
    ...
    def validate_report_data(self):
        '''
        Realiza validaciones sobre los reportes que usan como base este:
        - Verifica si existen cuentas con el mismo codigo que su grupo
        '''
        self.ensure_one()
        for account_id in self.account_ids:
            if account_id.code == account_id.group_id.code_prefix_start:
                raise ValidationError(
                    'La cuenta contable con codigo %s tiene el mismo codigo que el grupo'
                    ' contable %s, debe corregir uno de los códigos para que sea distinto. '
                    'Usualmente debería cambiar el codigo de la cuenta contable a %s01' % (
                        account_id.code, account_id.group_id.name, account_id.code
                    )
                )
            if not account_id.group_id and len(account_id.code) > 1 and not account_id.code.startswith('9'):
                account_id._compute_account_group()
                if not account_id.group_id:
                    raise ValidationError(
                        'La cuenta contable con codigo %s no tiene grupo y no hemos sido capaces '
                        'de asiganrle uno de los actuales. Debe crear un grupo y asignarlo a dicha cuenta. ' % (
                            account_id.code
                        )
                    )
    ...
    def _prepare_report_trial_balance(self):
        ...
        unaffected_earnings_account_groups = [self.unaffected_earnings_account.group_id.id]
        parent_id = self.unaffected_earnings_account.group_id.parent_id
        while parent_id:
            unaffected_earnings_account_groups.append(parent_id.id)
        ...
        return {
            ...
            "unaffected_earnings_account": self.unaffected_earnings_account.id if self.env.context.get(
                "unaffected_earnings_account", False
            ) else False,
            "unaffected_earnings_account_group_id": unaffected_earnings_account_groups if self.env.context.get(
                "unaffected_earnings_account", False
            ) else False,
            # "unaffected_earnings_account": self.unaffected_earnings_account.id,
            "account_financial_report_lang": self.env.lang,
            "show_signatures": self.env.context.get("show_signatures", False),
            "custom_report_name": self.env.context.get("custom_report_name", False),
            "pretty_rows": self.env.context.get("pretty_rows", False),
